...
def Getfromfile(stockdict:dict,player,gametime):
    with open('Assets/Stockdata/extradata.json','r') as file:
        ...
        if data:
            ...
            player.options = [OptionAsset(stockdict[option[0]],option[1],option[2],option[3],option[4],option[5],porfolioPercent=option[6],ogprice=option[7],color=tuple(option[8])) for option in data[2]]# options storage is [stockname,strikeprice,expirationdate,optiontype,quantity,ogprice]

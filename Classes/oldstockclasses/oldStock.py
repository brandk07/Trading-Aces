...
class Stock():
    ...
    def update_range_graphs(self,graphxsize):
        for key,value in self.graphrangeoptions.items():
            condensefactor = int(value/graphxsize)
            if len(self.pricepoints)/condensefactor > len(self.graphrangelists[key]):#if the amount of points that should be in the list is greater than the amount of points in the list
                self.graphrangelists[key].append(self.pricepoints[-1])#add the last point to the list
            elif len(self.pricepoints)/condensefactor < len(self.graphrangelists[key]):#if the amount of points that should be in the list is less than the amount of points in the list
                self.graphrangelists[key].pop(0)#remove the first point from the list
    ...
    def draw(self,screen:pygame.Surface,player:object,startingpos,endingpos,stocklist):
        ...
        for i,value in enumerate(graphingpoints):
            if value >= len(self.pricepoints)-1 or i >= len(graphingpoints)-1 or int(graphingpoints[i+1]) >= len(self.pricepoints):
                pass#if last one in list or i is too great then don't draw line
            elif myinterated == 115:
                ...
                end_time = timeit.default_timer()
                print(f"Execution time2: {end_time - start_time} seconds")
            else:

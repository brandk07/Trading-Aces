...
class BankMenu(Menu):
    def __init__(self,stocklist,gametime,player,transactions,tmarket) -> None:
        self.icon = pygame.image.load(r'Assets\Menu_Icons\bankIcon.jpg').convert_alpha()
        self.icon = pygame.transform.scale(self.icon,(140,100))
        super().__init__(self.icon)
        self.menuSelection = MenuSelection((200,105),(520,120),["Overview","Investments","Loans"],45)
        self.overView = OverView(player,transactions)
        self.investScreen = InvestmentScreen(stocklist,gametime,player,tmarket)
...
class InvestmentScreen:
    def __init__(self,stocklist,gametime,player,tmarket) -> None:
        self.player = player

...
class OptionAsset(Asset):
    ...
    def getValue(self,bypass=False,fullvalue=True):
        ...
        if self.daysToExpiration() < 0:
            ...
            # self.playerObj.cash += self.option.getPrice(method="BSM",iteration=1)*self.quantity
            # self.playerObj.options.pop(self.playerObj.options.index(self))
            if self in self.playerObj.options:
                self.playerObj.exerciseOption(self)

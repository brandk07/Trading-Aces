...
class OptionAsset(Asset):
    def __init__(self,player,stockobj,strikePrice:int,expirationDate:datetime.datetime,option_type:str,creationdate:str,quantity:int,porfolioPercent=None,ogprice=None,color=None) -> None:
    ...
    def __eq__(self,other):
        if not isinstance(other,OptionAsset):
            return False
        return [self.stockobj,self.strikePrice,self.option_type,self.expirationDate,self.date,self.ogvalue,self.portfolioPercent] == [other.stockobj,other.strikePrice,other.option_type,other.expirationDate,other.date,other.ogvalue,self.portfolioPercent]
    ...
    def copy(self):
        return OptionAsset(self.playerObj,self.stockobj,self.strikePrice,str(self.expirationDate),self.option_type,str(self.date),self.quantity,self.portfolioPercent,ogprice=self.getValue(bypass=True))
...
#         self.option = Op(european=True,kind=self.option_type,s0=float(self.stockobj.price)*100,k=self.strikePrice*100,t=self.expirationDate,sigma=calculate_volatility(tuple(self.stockobj.graphs['1Y'])),r=0.05)
        # if ogprice:
...
#         return f'{self.name}'
    ...
    #     if not isinstance(other,OptionAsset):
    #         return False
    #     return [self.stockobj,self.strikePrice,self.option_type,self.expirationDate,self.date] == [other.stockobj,other.strikePrice,other.option_type,other.expirationDate,other.date]
...
#     def get_inputs(self):
#         return (self.option_type,self.stockobj.price,self.strikePrice,self.expirationDate,calculate_volatility(tuple(self.stockobj.graphs['1Y'])),0.05,)
    
#     # create a method to return an exact copy of the object
#     def copy(self) -> 'OptionAsset':        
#             return OptionAsset(self.stockobj,self.strikePrice,self.expirationDate,self.option_type,self.date,quantity=self.quantity,ogprice=self.getValue(bypass=True))

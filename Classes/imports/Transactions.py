...
class Transactions:
    ...
    def drawscroll(self,screen,coords:tuple,wh:tuple,mousebuttons:int):
        textlist = self.transactlog
        
        textinfo = []# stores the text info for the latter scroll [text,fontsize,color]
        textcoords = [[(10,10),(10,10+int(self.height/4)),(10,10+int(self.height/4)*2)] for i in range(len(textlist))]
        ...
        for i,text in enumerate(textlist):
            ...
            polytexts.append([text[0],40,(190,190,190)])
            polytexts.append([text[1],40,(190,190,190)])
        ...
        scrollmaxcoords = wh
        self.latterscroll.store_rendercoords(coords, scrollmaxcoords,self.height,0,0,updatefreq=10)

        # drawing the latter scroll and assigning the selected asset
        newselected = self.latterscroll.draw_polys(screen, coords, scrollmaxcoords, mousebuttons, 0,drawbottom=False)# draws the latter scroll and returns the selected asset

    def draw(self,screen,mousebuttons,coords:tuple,wh:tuple):
        # points = [(coords[0],coords[1]),(coords[0]+self.width,coords[1]),(coords[0]+self.width,wh[1]),(coords[0],wh[1])]
        # gfxdraw.filled_polygon(screen,points,(60,60,60,200))
        pygame.draw.rect(screen,(0,0,0),pygame.Rect(coords[0],coords[1],self.width,wh[1]-coords[1]),5)

        self.drawscroll(screen,(coords[0]+10,coords[1]+95),(wh[0]-20,wh[1]),mousebuttons)
